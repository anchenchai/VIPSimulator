...
echo -e 'wd="'"simgrid_files/"'"'>> $output
echo -e 'Lfc=${1:? need Lfc as paramter}' >> $output
echo -e 'catalog=${wd}${Lfc}\n' >> $output 

echo -e 'algo=${2:? need Algo as paramter}' >> $output
echo -e 'type=`echo ${Lfc} | cut -d "'"_"'" -f 1`\n' >> $output
...
# version=2: lcg-cp to choose Source
...
echo 'cmd="java -cp ../../bin:../../simgrid.jar \
 VIPSimulator"'>> $output
...
echo -e "\n" >> $output
echo 'echo "${algo} is used for ${catalog}" ' >> $output
echo 'for i in {1..1}
    do
      ...
      do
        ...
        run=$cmd" ${platform_file} simgrid_files/'${deployment_file2}' '${total_particle_number}' '${number_of_gate_jobs}' '${sos_time}' '${number_of_merge_jobs}' '${cpu_merge_time}' '${events_per_sec}' ${version} 10000000 ${catalog} ${algo} 0 ${verbose} ${flag}"
        echo -e "\\t\\t$run"
